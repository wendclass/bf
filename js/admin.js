...
/* ── Sauvegarde Firestore via window.FB (firebase-init.js compat SDK) ── */
async function fbSave(key, value) {
  try {
    if (window.FB) {
      await window.FB.save(key, value);
...
/* ── Chargement initial depuis Firestore via window.FB ── */
async function loadFromFirestore() {
  const keys = ['posts','projects','services','clients','testimonials',
                'team','footer','pageContent','messages','admins'];
  try {
    if (!window.FB) throw new Error('window.FB non disponible (firebase-init.js non chargé)');
    const data = await window.FB.loadAll(keys);
...
function initProjectsPanel(){
  ...
  // Prévisualisation locale → suggère le chemin assets
  ...
  previewInput?.addEventListener('change', () => {
    ...
    const prev = document.getElementById('project-thumb-preview');
    const f = files[0];
    if (thumbInput && !thumbInput.value) {
      thumbInput.value = 'assets/img/projects/' + f.name.toLowerCase().replace(/\s+/g,'-');
    }
  ...
  // Preview live quand l'URL du thumbnail change
  const thumbInput = document.getElementById('p-thumbnail');
  const prev = document.getElementById('project-thumb-preview');
  thumbInput?.addEventListener('input', () => {
    if (prev && thumbInput.value) { prev.src = thumbInput.value; prev.style.display = 'block'; }
...
function openProjectForm(p){
  ...
  const thumb = p?.thumbnail||(p?.images&&p.images[0])||p?.image||'';
  const thumbInput=document.getElementById('p-thumbnail');
  const prev=document.getElementById('project-thumb-preview');
  if(thumbInput) thumbInput.value=thumb;
  if(prev){prev.src=thumb||'';prev.style.display=thumb?'block':'none';}
  const behanceInput=document.getElementById('p-behance');
  if(behanceInput) behanceInput.value=p?.behance||'';
