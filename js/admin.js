...
function initProjectsPanel(){
  ...
  fileInput?.addEventListener('change',()=>{
    ...
    files.forEach(f=>{
      const r=new FileReader();
      r.onload=e=>{projectImages.push(e.target.result);loaded++;if(loaded===files.length)renderProjectImagesGrid();};
      r.readAsDataURL(f);
    });
...
function saveProject(){
  ...
  S.saveProjects(projects);closeProjectForm();renderProjectsList();
  feedback('projects-feedback',`Projet sauvegardé (${proj.images.length} image${proj.images.length!==1?'s':''}).`,'success');
}
