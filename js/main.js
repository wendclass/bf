...
/* ══ DEFAULT DATA ══ */
function getDefaultPosts(){return[{id:'001',title:'Pourquoi votre logo ne suffit pas à faire une marque',category:'Branding',date:'12 janvier 2025',time:'09:00',excerpt:"Un logo, c'est une signature. Une marque, c'est une promesse tenue dans le temps.",cover:'',coverGradient:'linear-gradient(135deg,#6600CC 0%,#0A0A0A 100%)',hashtags:['branding','identité visuelle'],authorId:'scott',content:`<p>Quand un entrepreneur me dit <strong>« j'ai besoin d'un logo »</strong>, je l'entends souvent dire autre chose : j'ai besoin qu'on me prenne au sérieux.</p><h2>Le logo : une signature, pas une promesse</h2><p>Votre logo est la partie visible de quelque chose de beaucoup plus grand. En dessous se trouvent vos valeurs, votre ton de communication, vos couleurs.</p><h2>Les marques qui durent ont une âme</h2><ul><li>Un logo sans charte graphique, c'est un nom sans visage</li><li>Une charte sans stratégie, c'est un visage sans personnalité</li></ul>`},{id:'002',title:'Le design premium est-il accessible aux PME africaines ?',category:'Stratégie',date:'28 janvier 2025',time:'10:30',excerpt:"La question revient souvent dans nos conversations. Et notre réponse est oui.",cover:'',coverGradient:'linear-gradient(135deg,#330066 0%,#1a0033 100%)',hashtags:['design','PME','Afrique'],authorId:'scott',content:`<p>La question revient souvent : <strong>« Le design de qualité, c'est réservé aux grandes entreprises, non ? »</strong></p><p>Non. Et cette idée reçue coûte cher aux PME africaines qui la croient.</p>`},{id:'003',title:'3 erreurs de charte graphique que font les startups',category:'Design',date:'10 février 2025',time:'08:00',excerpt:"Nous les voyons partout. Trois erreurs qui trahissent une marque avant même qu'elle parle.",cover:'',coverGradient:'linear-gradient(135deg,#111111 0%,#2d0066 100%)',hashtags:['charte graphique','startup'],authorId:'scott',content:`<p>Trois erreurs reviennent systématiquement.</p><h2>Erreur 1 : Trop de couleurs</h2><p>Une charte solide repose sur 2 à 3 couleurs maximum.</p>`}]}
function getDefaultProjects(){return[{id:'p001',title:'Koulba Coffee',category:'Identité Visuelle',year:'2024',month:'Mars',description:"Une torréfaction artisanale de Bobo-Dioulasso qui voulait exister sur les marchés premium.",images:[],gradient:'linear-gradient(135deg,#6600CC 0%,#0A0A0A 100%)',behanceUrl:''},{id:'p002',title:'Festival Sahel Sounds',category:'Direction Artistique',year:'2024',month:'Juillet',description:"Direction artistique complète du festival : affiche principale, déclinaisons digitales.",images:[],gradient:'linear-gradient(135deg,#330066 0%,#1a0033 100%)',behanceUrl:''},{id:'p003',title:'Yiré Studio',category:'Branding',year:'2025',month:'Janvier',description:"Studio de couture contemporain ouagalais.",images:[],gradient:'linear-gradient(135deg,#111111 0%,#2d0066 100%)',behanceUrl:''},{id:'p004',title:'Wend-Yam ONG',category:'Design Éditorial',year:'2024',month:'Octobre',description:"Rapport annuel d'activités pour une ONG d'agriculture familiale.",images:[],gradient:'linear-gradient(135deg,#1a0033 0%,#0a0a0a 100%)',behanceUrl:''},{id:'p005',title:'Class S',category:'Identité Visuelle',year:'2024',month:'Août',description:"Notre propre identité. Violet Impérial, Cormorant Garamond.",images:[],gradient:'linear-gradient(135deg,#6600CC 0%,#330066 100%)',behanceUrl:'https://www.behance.net/wendclass'},{id:'p006',title:'Saponé Roots',category:'Branding',year:'2025',month:'Février',description:"Valorisation d'un artisanat local. Tissu, terre, identité.",images:[],gradient:'linear-gradient(135deg,#2d0066 0%,#0a0a0a 100%)',behanceUrl:''}]}
...
function getDefaultTeam(){return[{id:'scott',name:'Scott Nana',role:'CEO & Brand Designer',bio:"Fondateur de Class S, Scott est passionné du design visuel et du branding.",photo:'',certs:['Brand Strategy','Graphic Design','Art Direction'],social:{linkedin:'',behance:'https://www.behance.net/wendclass'}}]}
...
function renderWorkPreviewCard(p, i){
  ...
  const delayAttr = i > 0 ? ` data-delay="${(i*0.1).toFixed(1)}"` : '';
  return `<article class="work-card work-card-${i+1} reveal"${delayAttr} onclick="location.href='works.html'" style="cursor:pointer">
    <div class="work-card-bg" style="${bg}"></div>
    <div class="work-card-overlay"><span>Voir →</span></div>
    <div class="work-card-info"><h3 class="work-card-title">${p.title}</h3><p class="work-card-cat">${p.category} · ${p.year}</p></div>
  </article>`;
}
...
/* ══ FAVICON — depuis assets/ ══ */
(function(){
  let lnk = document.getElementById('favicon');
  if (!lnk) { lnk = document.createElement('link'); lnk.id='favicon'; lnk.rel='icon'; document.head.appendChild(lnk); }
  lnk.href = 'assets/favicon.png';
})();

/* ══ LOGO — depuis assets/, adaptatif par contexte ══ */
function applyLogo(){
  ...
  document.querySelectorAll('.navbar-logo, .footer-logo').forEach(el => {
    ...
    if (img) {
      img.src = LOGO;
      img.onload  = () => { img.classList.add('loaded'); if(txt) txt.style.display='none'; };
      img.onerror = () => { img.classList.remove('loaded'); if(txt) txt.style.display=''; };
    }
  });
  // Sidebar admin
  ...
  if (sImg) {
    sImg.src = LOGO;
    sImg.onload  = () => { sImg.classList.add('loaded'); if(sTxt) sTxt.style.display='none'; };
    sImg.onerror = () => { sImg.classList.remove('loaded'); if(sTxt) sTxt.style.display=''; };
  }
}
applyLogo();
...
(function(){
  ...
  const imgEl = overlay.querySelector('.intro-logo-img');
  const txtEl = overlay.querySelector('.intro-logo-text');
  if (imgEl) {
    imgEl.src = 'assets/logo.png';
    imgEl.onload  = () => { imgEl.style.display='block'; if(txtEl) txtEl.style.display='none'; };
    imgEl.onerror = () => { imgEl.style.display='none';  if(txtEl) txtEl.style.display=''; };
  }
  setTimeout(() => {
...
(function(){
  ...
  async function tryLogin(){
    const inp=document.getElementById('admin-modal-input');const err=document.getElementById('admin-modal-err');if(!inp)return;
    const hash=await sha256(inp.value);
    if(hash===HASH){sessionStorage.setItem('cs_auth_user',JSON.stringify({type:'headmaster',username:'Headmaster',perms:['articles','projects','services','pages','clients','messages','media','footer','analytics','users','settings']}));window.location.href='sx9kp-admin.html';}
    else{inp.value='';inp.classList.add('err');setTimeout(()=>inp.classList.remove('err'),500);if(err){err.textContent='Mot de passe incorrect.';err.style.display='block';}}
  }
  document.addEventListener('DOMContentLoaded',()=>{
    document.querySelectorAll('.navbar-logo,.footer-logo').forEach(el=>{
      el.addEventListener('click',e=>{e.preventDefault();count++;if(timer)clearTimeout(timer);timer=setTimeout(reset,2500);if(count>=7){reset();showModal();}});
    });
...
(function(){
  ...
  loop();
  document.addEventListener('DOMContentLoaded',()=>{
    document.querySelectorAll('a,button,.work-card,.blog-card,.work-page-card,.filter-pill,.btn-download,.team-card,.cta-ghost,.cta-solid,.cta-solid-white,.calendly-float').forEach(el=>{
...
/* ══ CALENDLY — bouton flottant ══ */
(function(){
  const CALENDLY_URL = 'https://calendly.com/wendclass';
  // Injecter le CSS Calendly
  const link = document.createElement('link');
  link.rel = 'stylesheet';
  link.href = 'https://assets.calendly.com/assets/external/widget.css';
  document.head.appendChild(link);
  // Injecter le script Calendly
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    btn.className = 'calendly-float';
    btn.innerHTML = '<span class="calendly-float-icon">📅</span><span class="calendly-float-label">Réserver un call</span>';
    btn.setAttribute('aria-label', 'Réserver un appel sur Calendly');
...
(async function initDynamicContent() {
  let posts, projects, svcs, clients, tms, fd, pc;
  try {
    const FB = await (window._fbReady || Promise.resolve(null));
    if (FB) {
      const data = await FB.loadAll(['posts','projects','services','clients','testimonials','footer','pageContent']);
      posts    = data.posts        ?? getDefaultPosts();
      projects = data.projects     ?? getDefaultProjects();
  ...
  /* Works preview dynamique (3 premiers projets) */
  const worksGrid = document.querySelector('.works-grid');
  if (worksGrid) {
    const preview = projects.slice(0, 3);
    if (preview.length) {
      worksGrid.innerHTML = preview.map((p, i) => renderWorkPreviewCard(p, i)).join('');
      requestAnimationFrame(() => {
        worksGrid.querySelectorAll('.work-card').forEach((c, i) => {
        ...
        // Réobserver les animations scroll
        const obs = new IntersectionObserver(entries => {
          entries.forEach(e => { if(e.isIntersecting){ e.target.classList.add('visible'); obs.unobserve(e.target); } });
        }, { threshold:.12, rootMargin:'0px 0px -50px 0px' });
        worksGrid.querySelectorAll('.reveal').forEach(el => obs.observe(el));
      });
    }
  }
  ...
  if (blogGrid) {
    ...
    if (!visiblePosts.length) {
      blogGrid.closest('.blog-preview')?.style.setProperty('display','none');
    } else {
      blogGrid.innerHTML = visiblePosts.map(renderBlogCard).join('');
    }
  }
  ...
  if (svcGrid) {
    if (!svcs.length) {
      svcGrid.closest('.services')?.style.setProperty('display','none');
    } else {
      ...
      svcGrid.querySelectorAll('.service-card').forEach(el => obs.observe(el));
    }
  }
  ...
  if (track) {
    if (!clients.length) {
      track.closest('.clients-section')?.style.setProperty('display','none');
    } else {
  ...
  if (tmsGrid) {
    if (!tms.length) {
      tmsGrid.closest('.testimonials')?.style.setProperty('display','none');
    } else {
