...
/* ══ BLOG PREVIEW ══ */
(function(){
  const grid=document.getElementById('blog-preview-grid');if(!grid)return;
  let posts;try{posts=JSON.parse(localStorage.getItem('cs_posts'))||getDefaultPosts();}catch{posts=getDefaultPosts();}
  grid.innerHTML=posts.slice(0,3).map(renderBlogCard).join('');
  requestAnimationFrame(()=>{grid.querySelectorAll('.blog-card').forEach((c,i)=>{c.style.opacity='0';c.style.transform='translateY(20px)';c.style.transition='opacity .5s ease,transform .5s ease';setTimeout(()=>{c.style.opacity='1';c.style.transform='translateY(0)';},i*100);});});
})();

/* ══ SERVICES PREVIEW ══ */
(function(){
  const grid=document.getElementById('services-grid');if(!grid)return;
  let svcs;try{svcs=JSON.parse(localStorage.getItem('cs_services'))||getDefaultServices();}catch{svcs=getDefaultServices();}
  grid.innerHTML=svcs.map((s,i)=>`<article class="service-card reveal" data-delay="${(i*.1+.1).toFixed(1)}"><span class="service-icon-wrap">${s.icon}</span><h3 class="service-title">${s.title}</h3><p class="service-text">${s.text}</p></article>`).join('');
  // Re-observe
...
/* ══ CLIENTS MARQUEE ══ */
(function(){
  const track=document.getElementById('clients-track');if(!track)return;
  let clients;try{clients=JSON.parse(localStorage.getItem('cs_clients'))||getDefaultClients();}catch{clients=getDefaultClients();}
  if(!clients.length){track.closest('.clients-section')?.style.setProperty('display','none');return;}
  ...
  track.innerHTML=html+html; // doubled for infinite loop
})();

/* ══ TESTIMONIALS ══ */
(function(){
  const grid=document.getElementById('testimonials-grid');if(!grid)return;
  let tms;try{tms=JSON.parse(localStorage.getItem('cs_testimonials'))||getDefaultTestimonials();}catch{tms=getDefaultTestimonials();}
  if(!tms.length){grid.closest('.testimonials')?.style.setProperty('display','none');return;}
  grid.innerHTML=tms.map(t=>{
...
/* ══ FOOTER DYNAMIC ══ */
(function(){
  let fd;try{fd=JSON.parse(localStorage.getItem('cs_footer'));}catch{}
  if(!fd)return;
  ...
  if(li&&fd.columns?.[0]?.links){li.innerHTML=fd.columns[0].links.map(l=>`<li><a href="${l.url}">${l.icon?l.icon+' ':''}${l.label}</a></li>`).join('');}
})();

/* ══ PAGE CONTENT DYNAMIC ══ */
(function(){
  let pc;try{pc=JSON.parse(localStorage.getItem('cs_page_content'));}catch{}
  if(!pc)return;
