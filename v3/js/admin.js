...
const S={
  ...
  posts:()=>S.get('cs_posts',window.ClassS?.getDefaultPosts()||[]),
  savePosts:v=>S.set('cs_posts',v),
  projects:()=>S.get('cs_projects',window.ClassS?.getDefaultProjects()||[]),
  saveProjects:v=>S.set('cs_projects',v),
  services:()=>S.get('cs_services',window.ClassS?.getDefaultServices()||[]),
  saveServices:v=>S.set('cs_services',v),
  clients:()=>S.get('cs_clients',window.ClassS?.getDefaultClients()||[]),
  saveClients:v=>S.set('cs_clients',v),
  testimonials:()=>S.get('cs_testimonials',window.ClassS?.getDefaultTestimonials()||[]),
  saveTestimonials:v=>S.set('cs_testimonials',v),
  team:()=>S.get('cs_team',window.ClassS?.getDefaultTeam()||[]),
  saveTeam:v=>S.set('cs_team',v),
  footer:()=>S.get('cs_footer',window.ClassS?.getDefaultFooter()||{}),
  saveFooter:v=>S.set('cs_footer',v),
  pageContent:()=>S.get('cs_page_content',window.ClassS?.getDefaultPageContent()||{}),
  savePageContent:v=>S.set('cs_page_content',v),
  messages:()=>S.get('cs_messages',[]),
  saveMessages:v=>S.set('cs_messages',v),
  admins:()=>S.get('cs_admin_users',[]),
  saveAdmins:v=>S.set('cs_admin_users',v),
};
...
function makeImgUpload(containerId, onLoaded, opts={}){
  ...
  fileInp?.addEventListener('change',()=>{
    ...
    if(f.size>maxMB*1024*1024){alert(`Image trop grande (max ${maxMB} Mo). Compressez avant d'importer.`);return;}
    const r=new FileReader();
    r.onload=e=>{
      if(allowCrop){
        openCropModal(e.target.result,(croppedData)=>{setImage(croppedData);});
      } else {
...
function articleImageHandler(){
  ...
  input.onchange=()=>{
    ...
    if(f.size>2*1024*1024){alert('Image trop grande (max 2 Mo).');return;}
    const r=new FileReader();
    r.onload=e=>{
...
function initProjectsPanel(){
  ...
  projectImgUpload=makeImgUpload('project-img-upload',()=>{});
}
...
function openProjectForm(p){
  ...
  if(projectImgUpload)projectImgUpload.setData(p?.image||'');
}
function closeProjectForm(){document.getElementById('project-form-wrap').style.display='none';editingProjectId=null;}
...
function saveProject(){
  ...
  const proj={id:editingProjectId||Date.now().toString(),title,category:document.getElementById('p-category').value,year:document.getElementById('p-year').value,month:document.getElementById('p-month').value,image:projectImgUpload?projectImgUpload.getData():'',description:document.getElementById('p-description').value.trim(),gradient:'linear-gradient(135deg,#6600CC 0%,#0A0A0A 100%)'};
  ...
  S.saveProjects(projects);closeProjectForm();renderProjectsList();feedback('projects-feedback','Projet sauvegardé.','success');
}
...
function renderClients(){
  ...
  S.clients().forEach(c=>{
    ...
    fileInp?.addEventListener('change',()=>{
      const f=fileInp.files[0];if(!f)return;
      if(f.size>500*1024){alert('Logo trop grand (max 500 Ko).');return;}
      const r=new FileReader();r.onload=e=>{if(thumb){thumb.src=e.target.result;thumb.classList.add('loaded');}};r.readAsDataURL(f);
    });
  });
}
